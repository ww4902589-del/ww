...
Transfer the supplied `liv_jimeng_dream_journey` Wallpaper Engine Web wallpaper source into the repository and prepare a tightly scoped Codex handoff for v6. The next implementation must make the character's primary action a subtle head turn, retain two blinks per 12-second loop without the current ghosting, and strengthen starfield rotation, layered fragment drift, crystal flow, breathing, arm, hair and cloth motion while preserving all existing interactions.
...
- Run syntax and visual checks against the checked-out branch after restoring those assets.
- Follow `AGENTS.md` and `TASKS.md` to implement the v6 local-layer character and background motion.
- Validate the result inside Wallpaper Engine at high, medium and low quality, including the blink gate and seamless 12-second loop.
